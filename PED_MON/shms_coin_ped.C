...
void shms_coin_ped(Int_t runNumber, Int_t eventNumber)//MAIN FUNCTION
{
...
{ 
  ...
  TFile *f2= new TFile(Form("ROOTfiles/shms_coin_replay_production_%d_%d.root",run,nevents));  // no golden
...
void hodo(Int_t runNumber, Int_t eventNumber)
{
  ...
  TFile *f4 = new TFile(Form("ROOTfiles/shms_coin_replay_production_%d_%d.root",run,nevents));//Takes Current ROOTfile
...
{   
  ...
  const Int_t nevents  = eventNumber;
  TFile *f3 = new TFile(Form("ROOTfiles/shms_replay_production_%d_50000.root",goldRunNum));//Golden ROOTfile Detectors only
  TFile *f4 = new TFile(Form("ROOTfiles/shms_coin_replay_production_%d_%d.root",run,nevents));//Takes Current ROOTfileDouble_t mean1[1000];
  Double_t mean2[28];
  Double_t mean22[28];
